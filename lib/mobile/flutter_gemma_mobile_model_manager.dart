...
class MobileModelManager extends ModelFileManager {
  ...
  var _loraPath = 'lora.bin';
  var _modelPath = 'model.bin';
  ...
  Future<void> _loadAsset(String path, String target) =>
  ...
  Future<bool> get _shouldLoadModel async {
    ...
    if (completer != null) {
      return completer.isCompleted
          ? false
          : throw Exception('Model is already loading');
    }
  ...
  Future<bool> get _shouldLoadLora async {
    ...
    if (completer != null) {
      return completer.isCompleted
          ? false
          : throw Exception('Lora weights are already loading');
    }
  ...
  Stream<int> _loadModelWithProgressIfNeeded(
      Stream<int> Function() loadCallback) async* {
    if (await _shouldLoadModel) {
      final completer = _modelCompleter = Completer<bool>();
      try {
  ...
  Future<void> setModelPath(String path, {String? loraPath}) async {
    ...
    if (await isModelInstalled) {
      await Future.wait([
        ...
        if (loraPath != null)
          _loadLoraIfNeeded(() async => _userSetLoraPath = loraPath),
      ]);
    }
  }
  ...
  Future<void> installModelFromAsset(String path, {String? loraPath}) async {
    await Future.wait([
      ...
      if (loraPath != null)
        _loadLoraIfNeeded(() => _loadAsset(loraPath, _loraPath)),
    ]);
  }
  ...
  Future<void> downloadModelFromNetwork(String url, {String? loraUrl}) async {
    await Future.wait([
      ...
      if (loraUrl != null)
        _loadLoraIfNeeded(() => _loadAsset(loraUrl, _loraPath)),
    ]);
  }
  ...
  Stream<int> downloadModelFromNetworkWithProgress(String url,
    ...
    yield* _loadModelWithProgressIfNeeded(
        () => _streamNetwork(url, _modelPath));
    if (loraUrl != null) {
