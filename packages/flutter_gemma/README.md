...
No additional configuration required. `hook/build.dart` (Native Assets) downloads `LiteRtLm.dll` + companion DLLs + the DXC runtime (`dxil.dll`, `dxcompiler.dll` v1.9.2602) from the GitHub release on first build, verifies them via SHA256, and bundles them next to your `app.exe`. End users need nothing installed for CPU or GPU: since `flutter_gemma_litertlm` 1.7.1 the VC++ runtime is statically linked into those DLLs ([#456](https://github.com/DenisovAV/flutter_gemma/issues/456)). `PreferredBackend.npu` is the exception — its Intel OpenVINO/TBB DLLs are prebuilt by Intel and still need the redistributable.
