...
/// A chat you can talk to. A spoken question is one [VoiceSession] turn —
/// transcribe, answer, synthesize — and a typed one still goes straight to the
/// chat. Both land in the same conversation.
///
...
class _ChatPageState extends State<ChatPage> {
  ...
  /// never comes through here: its audio arrives as a [VoiceReplyAudioEvent].
  Future<void> _speak(String text) async {
    final tts = _tts;
    if (tts == null || text.trim().isEmpty) return;
    await _play(await tts.synthesize(text), tts.sampleRate);
  }

  /// Starts playing 16-bit mono [pcm]. Returns once playback has STARTED — the
  /// composer unlocks while the answer is still being read out.
  Future<void> _play(Uint8List pcm, int sampleRate) async {
    // A reply with nothing to pronounce — an emoji, say — comes back empty.
    ...
    // `play()` completes when playback ENDS, so it is not awaited here.
  ...
  Future<void> _startListening() async {
    ...
    // Talking over the answer stops it. Without this the microphone records
    // the app's own voice along with yours.
    await _player.stop();
  ...
  Future<void> _stopListening() async {
    ...
    try {
      ...
      await _voiceTurn(_pcm!.takeBytes());
    } catch (error) {
      if (mounted) {
      ...
      await for (final event in _session!.runTurn(audio)) {
        ...
        switch (event) {
          ...
          case VoiceErrorEvent():
            // Neither happens yet: nothing interrupts a turn until Step 5,
            // and the session reports a failed stage as a stream error.
            break;
        }
      }
    } catch (error) {
