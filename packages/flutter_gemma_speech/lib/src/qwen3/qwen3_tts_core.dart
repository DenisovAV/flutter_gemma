...
// this initializes the KV cache the decode loop (`_run_decode`, Task 2.3)
// threads forward one step at a time. This task only asserts the returned
// KV tensors' shapes; token correctness (matching the golden first
// codebook-0 token) is verified once `runDecode` exists.
//
...
// [Qwen3TtsCore.runPrefill] is not truly public API (no v1 consumer outside
// this package calls it directly) but is deliberately not name-mangled
// private either, so `qwen3_prefill_test.dart` — a different library — can
// drive it directly; annotated `@visibleForTesting` to say so, mirroring
  ...
  static Future<Qwen3TtsCore> load({
    ...
    try {
      ...
      final talker = loadLiteRtGraph(
        bindings,
        environment,
        _artifactPath(artifactPaths, 'talker_int4.tflite'),
        accelerator,
      );
