...
- **LiteRT-LM**: native libs from `native-v0.15.0` GitHub Release (upstream pin `2117fc43`). Android tarball bundles the Qualcomm QNN dispatch stack and Windows tarball bundles Intel NPU dispatch (`LiteRtDispatch.dll` + OpenVino runtime + TBB) for `PreferredBackend.npu` (Qualcomm Snapdragon / Intel LunarLake/PantherLake) — **neither is produced by the build**, both are carried forward byte-identical from the previous release. v0.15.0: fixes the Android OpenCL per-turn memory leak (LiteRT-LM #2699, #348/#402); **breaks the stream-callback ABI** (4-arg → 2-arg chunk object) with no compat path, handled by a runtime probe in `stream_proxy.c`. Windows discrete GPU (WebGPU/Dawn) is still broken upstream — use CPU/NPU on Windows (LiteRT-LM #2957, retested on v0.15.0).
...
- **Current Version**: core `flutter_gemma` `1.5.2`, `flutter_gemma_rag_sqlite` `1.1.0`, `flutter_gemma_rag_qdrant` `1.1.0`; `flutter_gemma_litertlm` `1.4.0`, `flutter_gemma_mediapipe` `1.0.4`, `flutter_gemma_embeddings` `1.0.4`, `flutter_gemma_speech` `0.4.1`; `flutter_gemma_agent` `0.1.0`, `flutter_gemma_builtin_ai` `0.1.0`
...
- Native libs fetched at build time by `flutter_gemma_litertlm/hook/build.dart` (the only hook that carries the version) from the `native-v0.15.0` GitHub release; SHA256-verified, bundled via Native Assets
