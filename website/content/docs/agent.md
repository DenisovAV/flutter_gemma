...
built without it throws `ArgumentError` instead of quietly answering as if
there were no image — the engines disagree on the unsupported case, and a
silent drop would give you a confident answer about a photo the model never
saw.
