...
class VisionEncoderValidator {
  ...
  static ValidationResult _validateSize(Uint8List imageBytes, VisionSpecs specs) {
    ...
    if (sizeInBytes > specs.maxFileSize) {
      return ValidationResult(
        isValid: false,
        encoderType: specs.encoderType,
        message: 'File size (${sizeInMB.toStringAsFixed(2)}MB) exceeds maximum (${specs.maxFileSizeMB}MB)',
    ...
    try {
      ...
      if (estimatedDimensions.width < specs.minWidth || 
        ...
        return ValidationResult(
          isValid: false,
          encoderType: specs.encoderType,
          message: 'Image dimensions too small: ${estimatedDimensions.width}x${estimatedDimensions.height}',
      ...
      if (estimatedDimensions.width > specs.maxWidth || 
        ...
        return ValidationResult(
          isValid: false,
          encoderType: specs.encoderType,
          message: 'Image dimensions too large: ${estimatedDimensions.width}x${estimatedDimensions.height}',
      ...
      return ValidationResult(
        isValid: true,
        encoderType: specs.encoderType,
        message: 'Dimensions ${estimatedDimensions.width}x${estimatedDimensions.height} are acceptable',
        suggestions: [],
      );
    } catch (e) {
      return ValidationResult(
  ...
  static bool _hasCorruptionIndicators(Uint8List imageBytes) {
    ...
    for (final count in patternCounts.values) {
      if (count > imageBytes.length * 0.01) { // More than 1% of the image
        return true;
      }
    }
