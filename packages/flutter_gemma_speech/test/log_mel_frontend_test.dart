...
void main() {
  ...
  group('computeNemoLogMel (golden vs Python NeMo-recipe reference)', () {
    test('matches the Python reference within tolerance for a 1s 440Hz tone', () {
      ...
      //
      // Convention note: this fixture pins the Bessel-corrected ddof=1
      // (sample std) z-score, matching real NVIDIA/NeMo and what
      // computeNemoLogMel now computes. It is (re)generated by the INDEPENDENT
      // numpy reference test/fixtures/gen_nemo_log_mel_golden.py — which
      // derives the z-score from numpy's own std, NOT from this Dart code — so
      // this test genuinely discriminates the normalization convention rather
      // than encoding it. Regenerate the fixture with that script if the
      // recipe ever changes.
