...
class _ChatPageState extends State<ChatPage> {
  ...
  /// The hits behind the reply currently on screen. Shown under it, because a
  /// grounded answer the user cannot check is not obviously better than an
  /// ungrounded one.
  List<RetrievalResult> _sources = const [];
  ...
  Future<void> _send() async {
    ...
    try {
      // GROUNDING. Everything the model is about to say about a recipe has to
      // come from the corpus, so retrieve first and hand the hits over as part
      // of the turn. The model never sees the vector store — it sees text.
      //
...
Question: $text''';
      ...
      if (mounted) setState(() => _sources = hits);
                    ...
                    children: [
                      for (final h in _sources)
                        Chip(
                          ...
                          label: Text(
                            '${RagStore.recipeFor(h)?.title ?? h.id} '
