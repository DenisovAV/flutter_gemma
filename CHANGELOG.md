...
- **Fix `modelFromAsset` install on Android + desktop** (#250): `large_file_handler` was getting a bare filename (Android wrote files into JVM cwd), and the macOS/Windows/Linux/web channel was missing entirely. Now passes an absolute target path and falls back to in-memory copy on `MissingPluginException`. Embedding (`localagents-rag`) and `.litertlm` FFI paths now fail fast with a typed message on non-arm64 Android (x86_64 emulator / armeabi-v7a) instead of a generic JVM crash — see new "Android Architecture Support" section in README.
