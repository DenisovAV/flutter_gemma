## 0.14.2
- **Gemma 4 escape-token leakage in tool_calls history (#248)**: when the SDK left `<|"|>` escape tokens in `tool_calls.arguments` strings, `InferenceChat` wrote the raw JSON straight into chat history. On the next turn the SDK echoed those tokens back to the model, and Gemma 4 started reproducing them in subsequent `tool_calls` arguments — corrupting string values and especially `enum` matching (e.g. `"<|\"|>sumAmount<|\"|>"` instead of `"sumAmount"`). 0.14.2 strips the tokens recursively (including nested maps and lists) before history persist via the new `SdkResponseParser.cleanRawForHistory(raw)` helper, so subsequent turns see clean inputs and the model converges to the canonical enum values. Fix applies to both sync (`generateChatResponse`) and streaming (`generateChatResponseAsync`) paths.
- **macOS install_name_tool fix (#247)**: `dart run` / `dart build_runner` / `flutter test` on a pure Dart library that depends on `flutter_gemma` failed with `install_name_tool: ... larger updated load commands do not fit`. Root cause: Dart Native Assets in JIT mode rewrites each bundled dylib's `LC_ID_DYLIB` to a long absolute path inside `.dart_tool/lib/`, but the three Apple accelerator dylibs Google ships (`libGemmaModelConstraintProvider`, `libLiteRtMetalAccelerator`, `libLiteRtTopKMetalSampler`) were linked without `-Wl,-headerpad_max_install_names`, leaving only 32 bytes of slack — not enough for an 80–110-char path. 0.14.2 sidesteps this on macOS by skipping those three from the Native Assets asset list (`hook/build.dart`) and copying them into `App.app/Contents/Frameworks/<X>.framework/` from `example/macos/Podfile`'s `post_install` instead, then re-pointing `LiteRtLm.dylib`'s `LC_LOAD_DYLIB` reference at the new framework path. iOS / Linux / Windows / Android are unaffected (their bundling pipelines don't trigger the bug).
- **macOS Podfile setup (BREAKING for macOS app projects)**: macOS app projects must update their `macos/Podfile` `post_install` block to the new snippet (see [README → macOS Setup](README.md#macos-setup)). Without it, `flutter build macos` succeeds but companion frameworks are missing from the app bundle and `litert_lm_engine_create` fails at runtime with `Library not loaded: @rpath/libGemmaModelConstraintProvider.dylib`. iOS, Linux, Windows, Android and pure-Dart `flutter_gemma` consumers don't need any change.
- **Performance debug logs**: `[LiteRtLmFfi/perf]`, `[FfiInferenceModel/perf]`, `[FfiInferenceModelSession/perf]`, `[FlutterGemmaMobile/perf]` lines now break out the wall-clock cost of dylib loading, `litert_lm_engine_create`, conversation creation, prefill, and per-chunk decode — visible in `flutter run` / Xcode console for both debug and release builds. After successful `engine_create` the SDK's `stderr` (model load, accelerator init, sampler dlopen attempts, KV-cache prefill) is auto-dumped through `debugPrint` on iOS/macOS/Linux debug builds.
- **iPhone integration test for Gemma 4 timing**: `example/integration_test/ios_gemma4_engine_create_timing.dart` reproduces the `eav-solution` config from #245 and prints stage-by-stage timings — useful for benchmarking and regression triage.
- **`build-native` skill**: codified the post-build verification checklist that catches the bugs we already shipped at users (App Store ITMS-90432, headerpad mismatch, x86_64-in-iOS-arm64-prebuilts, Phase 8 patch markers).

## 0.14.1
- **Web build fix (#244)**: 0.14.0 broke web compilation by statically importing `core/ffi/litert_lm_client.dart` (which imports `dart:ffi`) through the `flutter_gemma_interface.dart` → `mobile/flutter_gemma_mobile.dart` chain. JS/Wasm targets cannot compile `dart:ffi`. Added conditional imports in `mobile/flutter_gemma_mobile.dart` that swap `litert_lm_client.dart` and `ffi_inference_model.dart` for `*_stub.dart` shims on `dart.library.js_interop`. The web plugin (`FlutterGemmaWeb`) registers itself as `FlutterGemmaPlugin.instance` before any FFI code path runs, so the stubs' constructors (which throw `UnsupportedError`) are never actually invoked on web.
...
- **macOS / iOS setup**: requires a small `post_install` block in your `Podfile` to symlink `lib*.dylib` next to the bundled `.framework`s — `gpu_registry` calls `dlopen` by basename. See the [macOS Setup section in README](README.md#macos-setup) for the exact snippet (iOS works via the same pattern in `example/ios/Podfile`)
