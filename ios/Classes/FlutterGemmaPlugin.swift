...
public class FlutterGemmaPlugin: NSObject, FlutterPlugin {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    let channel = FlutterMethodChannel(
      name: "flutter_gemma", binaryMessenger: registrar.messenger())
  ...
  @MainActor public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "init":
      if let arguments = call.arguments as? [String: Any],
        ...
        let temperature = arguments["temperature"] as? Float,
        let randomSeed = arguments["randomSeed"] as? Int,
        let topK = arguments["topK"] as? Int,
        let loraPath = arguments["loraPath"] as? String?,
        let supportedLoraRanks = arguments["supportedLoraRanks"] as? [Int]?
      {
        do {
          inferenceController = try InferenceController(
            maxTokens: maxTokens, temperature: temperature, randomSeed: randomSeed, topK: topK, loraPath: loraPath,
            supportedLoraRanks: supportedLoraRanks)
        } catch {
          result(
            FlutterError(
              code: "ERROR", message: "Failed to initialize gemma: \(error.localizedDescription)",
              details: nil))
        }
        result(true)
      } else {
        ...
        let prompt = arguments["prompt"] as? String
      {
        do {
          if let response = try inferenceController?.sendMesssage(prompt) {
            ...
            result(
              FlutterError(
                code: "UNAVAILABLE", message: "Inference model could not generate a response",
                details: nil))
          }
        } catch {
          result(
            FlutterError(
              code: "ERROR", message: "Failed to generate response: \(error.localizedDescription)",
              details: nil))
        }
      } else {
        result(
          FlutterError(
            code: "BAD_ARGS", message: "Bad arguments for 'getGemmaResponse' method", details: nil))
      }
