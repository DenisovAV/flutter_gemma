...
void main() {
  ...
  group('Metal accelerator (Bug A)', () {
      ...
      (tester) async {
        ...
        try {
          ...
          expect(
            ...
            isNotEmpty,
            reason:
                'First message must produce a response. '
    ...
    testWidgets(
      'A2: three consecutive messages complete without crash',
      (tester) async {
        await registerTestEngines();
        ...
        try {
          ...
          for (var i = 1; i <= 3; i++) {
            ...
            expect(
              ...
              isNotEmpty,
              reason:
                  'Message $i must produce a response. '
                  'SIGSEGV here = Metal accelerator missing (Bug A).',
            );
          }
        } finally {
  ...
  group('callbackFlow race condition (Bug B)', () {
    ...
    testWidgets(
      'B1: sequential open/chat/close cycles do not crash the server',
      (tester) async {
        await registerTestEngines();
    ...
    testWidgets(
      'B2: mid-stream disconnect does not crash server',
      (tester) async {
        await registerTestEngines();
        ...
        await model1.createChat().then((chat) async {
          await chat.addQueryChunk(
            const Message(
              text:
                  'Write a very long story about a dragon. At least 500 words.',
              isUser: true,
            ),
          );
        ...
        try {
          ...
          expect(
            ...
            isNotEmpty,
            reason:
                'Server must still respond after mid-stream disconnect. '
  ...
  group('Issue #219 reproduction (Qwen 2.5)', () {
    ...
    testWidgets(
      'C1: first message on Qwen 2.5 does not crash',
      (tester) async {
        await registerTestEngines();
        ...
        try {
          ...
          expect(
            ...
            isNotEmpty,
            reason:
                'First message on Qwen 2.5 must complete without SIGSEGV. '
    ...
    testWidgets(
      'C2: five consecutive messages on Qwen 2.5 do not crash',
      (tester) async {
        await registerTestEngines();
    ...
    testWidgets(
      'C3: disconnect during prefill does not crash server',
      (tester) async {
        await registerTestEngines();
